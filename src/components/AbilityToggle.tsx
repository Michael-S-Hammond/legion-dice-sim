...
function AbilityToggle(props: AbilityProps) : JSX.Element {
    ...
    return (
        <div key={`${props.id}-abilitytoggle`} className={`${ props.visible ? 'd-flex collapse.show' : 'collapse'} justify-content-center my-3 custom-control custom-switch`}>
            ...
            { props.alert &&
                <button id={`${props.id}-abilitytoggle-popover`} type="button" className="btn btn-outline-secondary popover-button" data-container="body" data-toggle="popover" data-trigger="focus" data-placement="top" data-content={props.alert}>
